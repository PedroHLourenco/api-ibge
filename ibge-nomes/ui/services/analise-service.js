...
class AnaliseService {
    ...
    async processarRankingNomesLocalidade(localidade) {
        ...
        try {
            ...
            // Para cada nome, obtém a evolução por década usando o método de ranking por década
            const decadas = ['1930', '1940', '1950', '1960', '1970', '1980', '1990', '2000', '2010'];
            
            // Primeiro, obtemos os dados de cada nome individualmente
            const evolucaoNomes = await Promise.all(
                topNomes.map(async (item) => {
                    // Obtém dados do nome em todas as décadas disponíveis
                    const dadosNome = await this.ibgeService.getFrequenciaPorNomeELocalidade(item.nome, localidade);
                    
                    // Também obtemos dados específicos por década para garantir cobertura completa
                    const dadosPorDecada = await Promise.all(
                        decadas.map(async (decada) => {
                            try {
                                // Tenta obter dados específicos para esta década
                                const rankingDecada = await this.ibgeService.getRankingNomesPorDecada(decada, localidade);
                                // Procura o nome atual no ranking desta década
                                const nomeNaDecada = rankingDecada[0]?.res?.find(n => n.nome === item.nome);
                                return nomeNaDecada ? { 
                                    periodo: `${decada}[`, 
                                    frequencia: nomeNaDecada.frequencia,
                                    proporcao: nomeNaDecada.frequencia / rankingDecada[0].res[0].frequencia 
                                } : null;
                            } catch (error) {
                                console.log(`Erro ao buscar dados para ${item.nome} na década ${decada}:`, error);
                                return null;
                            }
                        })
                    );
                    
                    // Combina os dados do nome com os dados específicos por década
                    const evolucaoCompleta = [...dadosNome[0].res];
                    
                    // Adiciona dados de décadas que possam estar faltando
                    dadosPorDecada.forEach(dadoDecada => {
                        if (dadoDecada && !evolucaoCompleta.some(d => d.periodo === dadoDecada.periodo)) {
                            evolucaoCompleta.push(dadoDecada);
                        }
                    });
